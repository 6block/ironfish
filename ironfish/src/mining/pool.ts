...
const RECALCULATE_TARGET_TIMEOUT = 20000
...
export class MiningPool {
    ...
    if (options.rpcProxy) {
      ...
      this.logger.info(
        `MiningRpcNode mounted on pool: ${JSON.stringify(
          this.rpcProxy.map((a) => `${a.describe()}`),
        )}`,
      )
    } else {
  ...
  async start(): Promise<void> {
    ...
    if (this.stratum.enableTlsPool) {
      ...
      this.logger.info(
        `Starting tls stratum server v${this.stratum.version} on ${this.stratum.tlsHost}:${this.stratum.tlsPort}`,
      )
    }
    this.stratum.start()
            ...
            this.rpcProxy.map(async (rpc) => {
              if (rpc.isConnected) {
                ...
                if (submitResponse.content.added) {
                  this.logger.info(
                    `Block submitted to rpc node ${JSON.stringify(
                      rpc.describe(),
                    )} successfully!`,
                  )
                  submitResp += 1
        ...
        try {
          if (await rpc.tryConnect()) {
            ...
            this.connectTimeout = setTimeout(
              () => void this.startConnectingMiningRpc(rpc.describe()),
              5000,
            )
          }
        } catch (e) {
          this.logger.warn(`Error when connect to rpc node ${rpc.describe()}`)
        }
      }),
    )
      ...
      this.rpcProxy.map(async (rpc) => {
        if (rpc.describe() === connect) {
          try {
            ...
            this.logger.warn(`Error when connect to rpc node ${connect}`)
          }
        }
      }),
    )
        ...
        if (rpc.isConnected) {
          try {
            for await (const payload of rpc.blockTemplateStream().contentStream(true)) {
              Assert.isNotUndefined(payload.previousBlockInfo)
              this.processTemplate(payload, `${rpc.describe()}`)
            }
          } catch (e) {
            this.logger.warn(
              `Exception when getBlockTemplate on ${rpc.describe()}, ${JSON.stringify(e)}`,
            )
          }
        }
      }),
